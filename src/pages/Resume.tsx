...
const LoadingScreen = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-cream-50 via-amber-100 to-yellow-50">
      {/* Main Container */}
      <div className="w-full max-w-sm p-8 bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl border border-amber-200 text-center space-y-6">
        {/* Animated Logo */}
        ...
        {/* Title */}
        <h2 className="text-3xl font-bold text-gray-800 bg-gradient-to-r from-amber-600 to-orange-500 bg-clip-text text-transparent">
          Loading Professional Resume
        </h2>

        {/* Subtitle */}
        <p className="text-gray-600 text-sm">
          Preparing exceptional content just for you...
        </p>
        ...
        {/* Loading Tip */}
        <div className="bg-white/70 backdrop-blur-sm p-4 rounded-lg border border-amber-200 text-left">
          <p className="text-xs text-gray-600 italic">
            "Did you know? This resume is powered by React, TypeScript, and Tailwind CSS!"
          </p>
        </div>
...
const SectionHeader = ({ title, subtitle, icon }: { title: string; subtitle: string; icon: React.ReactNode }) => (
  <div className="text-center mb-16">
    <div className="flex justify-center mb-4">
      <Badge className="px-4 py-2 bg-amber-100 text-amber-800 hover:bg-amber-200 transition-colors">
        {icon}
      </Badge>
    </div>
    <h2 className="text-4xl lg:text-5xl font-bold mb-4 bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent">
      {title}
    </h2>
...
const StatCard = ({ value, label, icon }: { value: string; label: string; icon: React.ReactNode }) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 text-center hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
...
const SkillCard = ({ title, icon, skills, gradient }: { title: string; icon: React.ReactNode; skills: string[]; gradient: string }) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-xl transition-all duration-300 hover:scale-[1.01]">
...
const ExperienceCard = ({ experience }: any) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-xl transition-all duration-300 hover:scale-[1.01] group">
    <div className={`h-2 bg-gradient-to-r ${experience.gradient}`}></div>
    <CardHeader>
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-4">
          <div className={`p-3 rounded-lg bg-gradient-to-br ${experience.gradient}`}>
...
const EducationCard = ({ education }: any) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-xl transition-all duration-300 hover:scale-[1.01] group">
    <div className={`h-32 bg-gradient-to-br ${education.gradient} relative overflow-hidden`}>
...
const AchievementCard = ({ achievement }: any) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-lg transition-all duration-300 hover:scale-[1.01]">
    <CardContent className="p-6">
      <div className="flex items-start space-x-4">
        <div className={`p-3 rounded-lg bg-gradient-to-br ${achievement.gradient}`}>
...
const CertificationCard = ({ certification }: any) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-lg transition-all duration-300 hover:scale-[1.01]">
    <CardContent className="p-6">
      <div className="flex items-start space-x-4">
        <div className={`p-3 rounded-lg bg-gradient-to-br ${certification.gradient}`}>
          <certification.icon className="h-6 w-6 text-white" />
        </div>
        <div className="flex-1">
          <h4 className="font-semibold text-gray-800 mb-1">{certification.title}</h4>
...
const FutureGoalCard = ({ goal }: any) => (
  <Card className="bg-white/70 backdrop-blur-sm border border-amber-200 hover:shadow-xl transition-all duration-300 hover:scale-[1.01] group">
    <div className={`h-24 bg-gradient-to-br ${goal.gradient} relative overflow-hidden`}>
      <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors flex items-center justify-center">
...
const Resume = () => {
  ...
  const resumeData = {
    ...
    skills: [
      {
        ...
        icon: <Monitor className="h-6 w-6 text-amber-600" />,
      ...
      {
        title: 'DevOps & Tools',
        icon: <Terminal className="h-6 w-6 text-amber-600" />,
    ...
    stats: [
      { value: '10+', label: 'Projects Completed', icon: <Briefcase className="h-8 w-8 text-amber-600 mx-auto mb-2" /> },
      { value: '2+', label: 'Years Experience', icon: <Calendar className="h-8 w-8 text-amber-600 mx-auto mb-2" /> },
      { value: '15+', label: 'Happy Clients', icon: <Users className="h-8 w-8 text-amber-600 mx-auto mb-2" /> },
      { value: '5+', label: 'Certifications', icon: <Award className="h-8 w-8 text-amber-600 mx-auto mb-2" /> }
    ]
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-cream-50 via-cream-100 to-amber-50 text-foreground">
          ...
          {[20, 40, 80].map((top, i) => (
            <div
              ...
              style={{
                ...
                right: i === 1 ? '10%' : 'auto',
                width: i === 0 ? '18rem' : i === 1 ? '24rem' : '20rem',
                height: i === 0 ? '18rem' : i === 1 ? '24rem' : '20rem',
        ...
        <div className="container mx-auto text-center relative z-10">
          <div className="animate-fade-in space-y-6">
            ...
            <p className="text-xl lg:text-2xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              Full-Stack Developer & Tech Educator passionate about building scalable solutions and mentoring the next generation of developers.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4 mt-8">
              ...
              <a href="#contact">
                <Button
                  size="lg"
                  variant="outline"
                  className="border-amber-300 text-gray-700 hover:bg-amber-50 backdrop-blur-sm bg-white/50 hover:border-amber-400 transition-all duration-300 hover:scale-105 px-8 py-4 group"
                >
                  Get in Touch
                  <ExternalLink className="ml-2 h-5 w-5 group-hover:translate-x-0.5 transition-transform" />
                </Button>
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto">
          <SectionHeader
            ...
            subtitle="Comprehensive skill set covering modern web development technologies and tools."
      ...
      <section className="py-20 px-6 bg-white/50 backdrop-blur-sm">
        <div className="container mx-auto">
          <SectionHeader
            ...
            subtitle="Delivering impactful solutions through full-stack development and collaboration."
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto">
          <SectionHeader
            title="Education"
            subtitle="Academic journey combining formal education with continuous learning."
      ...
      <section className="py-20 px-6 bg-white/50 backdrop-blur-sm">
        <div className="container mx-auto">
          <SectionHeader
            ...
            subtitle="Recognition for excellence and continuous professional development."
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
              <h3 className="text-2xl font-bold mb-6 text-gray-800 flex items-center">
                <Star className="h-6 w-6 mr-2 text-amber-600" />
                Key Achievements
              </h3>
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto">
          <SectionHeader
            title="Future Goals"
            subtitle="Aspirations to push boundaries in technology, education, and innovation."
      ...
      <section className="py-20 px-6 bg-white/50 backdrop-blur-sm">
        <div className="container mx-auto text-center">
          <Card className="bg-white/70 backdrop-blur-sm p-8 rounded-2xl border border-amber-200 animate-slide-up shadow-sm hover:shadow-md transition-shadow">
            <CardHeader>
              ...
              <CardDescription className="text-lg text-gray-600 max-w-2xl mx-auto">
                ...
                Whether you need a full-stack developer, technical consultant, or mentor, I'm here to help.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <Button
                  size="lg"
                  className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] px-8 py-4 group"
                >
                  <a href="/resume.pdf" download className="flex items-center">
                ...
                <a href="#contact">
                  <Button
                    size="lg"
                    variant="outline"
                    className="border-amber-300 text-gray-700 hover:bg-amber-50 backdrop-blur-sm bg-white/50 hover:border-amber-400 transition-all duration-300 hover:scale-[1.02] px-8 py-4 group"
                  >
                    Schedule a Call
                    <ExternalLink className="ml-2 h-5 w-5 group-hover:translate-x-0.5 transition-transform" />
                  </Button>
